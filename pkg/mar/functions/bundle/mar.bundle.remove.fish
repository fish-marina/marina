function mar.bundle.remove
    set -l bundle $MAR_CONFIG/bundle

    if test -L $MAR_CONFIG/bundle
      set bundle (readlink $MAR_CONFIG/bundle)
    end
    ...
    if test -f $bundle
      ...
      for record in $bundle_contents
        ...
        set record_name (mar.packages.name $record_name_or_url)
