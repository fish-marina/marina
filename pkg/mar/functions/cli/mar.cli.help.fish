function mar.cli.help
  set -l doc_root $MAR_PATH/docs/cli
  set -l doc $doc_root/mar.adoc

  # If a command was given, find a help document for it.
  if set -q argv[1]
    if not set command (mar.command $argv[1])
      echo (mar::err)"Unknown command: $argv[1]"(mar::off) >&2
