function mar.index.update -d 'Update package indexes'
  # Only update if the index is missing.
  if contains -- --if-missing $argv
    set -l paths (mar.index.path)/*
  ...
  for list in {$MAR_PATH,$MAR_CONFIG}/repositories
    test -f $list
  ...
  while set -q repositories[1]
    ...
    set -l path (mar.index.path)/$repositories[3]
