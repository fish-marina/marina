function describe_basic_tests
  ...
  function it_has_a_help_command
    ...
    echo $output | grep -Eq "destroy.+Uninstall Marina"
    echo $output | grep -Eq "doctor.+Troubleshoot Marina"
    ...
    echo $output | grep -Eq "update.+Update Marina"
    echo $output | grep -Eq "version.+Display version and exit"
    assert 0 = $status
  end

  function it_has_a_doctor_command
    set -l output (mar doctor)
    assert 0 = $status
    assert -n (echo $output | grep "Marina version")
    assert -n (echo $output | grep "Checking for a sane environment...")
  end

  function it_installs_packages
    set -l remove_output (mar remove apt 2> /dev/null)
    set -l install_output (mar install apt)
    assert 0 = $status
    assert -n (echo $install_output | grep "apt successfully installed.")
  end

  function it_removes_packages
    set -l install_output (mar install apt 2> /dev/null)
    set -l remove_output (mar remove apt)
    assert 0 = $status
